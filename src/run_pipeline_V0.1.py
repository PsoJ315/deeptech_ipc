...
def main():
    ...
    term_doc_df = build_term_doc_table(df, extractor)
    term_doc_path = tables_dir / "term_doc_table.csv"
    term_doc_df.to_csv(term_doc_path, index=False, encoding="utf-8-sig")
    print(f"[save] {term_doc_path}")

    print("=== STEP 3: Frequency Tables ===")
    freq_tables = build_term_frequency_tables(term_doc_df)
    ...
    candidate_df = build_term_candidate_table(term_doc_df, df, top_n_context=3)
